...
//        } else {
            ...
            if ($order_info) {
                $now_time = date("Y-m-d H:i:s",time()-13*60*60);
                $now_time = strtotime($now_time);
                $show_time = strtotime($order_info['date_added']);
                $dur = $now_time - $show_time;
                // Do not allow to delete the order if it exceeds 30minutes.
                if ($dur > 1800) {
                    $json['error'] = $this->language->get('error_exceed_30min');
                }
                else {
                    $this->model_checkout_order->cancelOrder($order_id);
                    $json['success'] = $this->language->get('text_success');
                }
            } else {
                $json['error'] = $this->language->get('error_not_found');
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (isset($this->session->data['user'])){
			    $user_id = $this->session->data['user']['user_id'];
            }

			if ($order_info) {
				$this->model_checkout_order->addOrderHistory($order_id, $this->request->post['order_status_id'],
                    $this->request->post['comment'], $this->request->post['notify'], $this->request->post['override']);
